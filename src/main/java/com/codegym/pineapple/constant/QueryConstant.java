...
public class QueryConstant {
    ...
    public static final String QUERY_FIND_ALL_PRODUCT = "select pd.id, c.name, p.name, pd.color, pd.amount, pd.price, pd.description, pd.product_id from product_details pd  inner join products p on pd.product_id = p.id inner join categories c on p.category_id = c.id limit ? offset ?;";
    public static final String QUERY_EDIT_PRODUCT = "update product_details set color=?, amount=?, price=? where id=?;";
